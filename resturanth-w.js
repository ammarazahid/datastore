let kolachi = {
    name: "Kolachi Resturant",
    location: "D.H.A phase 6",
    address: "Creak Side، Abdul Sattar Edhi Ave, D.H.A. Phase 8 Zone C Phase 8 Karachi, Sindh 75500",
    hours: [
        "Thursday	7pm–1am",
        "Friday	7pm–1am",
        "Saturday	7pm–1am",
        "Sunday	7pm–1am",
        "Monday	7pm–1am",
        "Tuesday	7pm–1am",
        "Wednesday	7pm–1am"
    ],
    phone: "(021) 111 111 001",
    employees: [
        {
            name: "Mr. Ahmed Khan",
            salary: 10000,
            age: 24,
            Gender: "Male",
            maritalStatus: "unmarried",
            qualification: "B.A",
            designation: "waiter",
            contact: "03490876541",
            address: "sygiosfja"
        },
        {
            name: "saad sheikh",
            fatherName: "zaid sheikh",
            salary: 10000,
            age: 22,
            maritalStatus: "unmarried",
            Gender: "Male",
            qualification: "Intermediate",
            designation: "waiter",
            contact: "04568907345",
            address: "h#12,st#4,area karachi"
        },
        {
            name: "dabeer khan",
            fatherName: "sajjid",
            Gender: "Male",
            maritalStatus: "unmarried",
            salary: 10000,
            age: 23,
            qualification: "Intermediate",
            designation: "waiter",
            contact: "09876543211",
            address: "h#4,st#6,area karachi"
        },
        {
            name: "Umair sheikh",
            fatherName: "Sheikh saeed",
            Gender: "Male",
            maritalStatus: "unmarried",
            salary: 10000,
            age: 25,
            qualification: "Matriculation",
            contact: "03409876543",
            designation: "waiter",
            address: "h#12,st#4,area karachi"
        },
        {
            name: "Bilal khan",
            Gender: "Male",
            fatherName: "umar khan",
            salary: 10000,
            maritalStatus: "unmarried",
            age: 19,
            qualification: "Matriculation",
            designation: "waiter",
            contact: "03769098090",
            address: "h#4,st#6,area karachi"
        },
        {
            name: "Bismillah khan",
            fatherName: "majeed khan",
            salary: 10000,
            maritalStatus: "unmarried",
            age: 19,
            Gender: "Male",
            qualification: "Matriculation",
            designation: "waiter",
            contact: "03456543211",
            address: "karachi"
        },
        {
            name: "Abdullah",
            fatherName: "khurram",
            maritalStatus: "unmarried",
            salary: 10000,
            age: 21,
            Gender: "Male",
            qualification: "Matriculation",
            designation: "waiter",
            contact: "0345987654",
            address: "defence"
        },
        {
            name: "mahmood",
            fatherName: "iqrar",
            Gender: "Male",
            maritalStatus: "unmarried",
            salary: 15000,
            age: 23,
            designation: "security guard",
            qualification: "matric",
            contact: "09876543211",
            address: "Tin talwar"
        },
        {
            name: "Ammad khan",
            fatherName: "ahmed",
            Gender: "Male",
            maritalStatus: "unmarried",
            salary: 16000,
            designation: "security guard",
            age: 21,
            qualification: "Intermediate",
            contact: "03976543211",
            address: "Quaidabad"
        },
        {
            name: "Ali Ahmed",
            fatherName: "Ahmed shahzad",
            Gender: "Male",
            maritalStatus: "unmarried",
            salary: 15000,
            age: 24,
            designation: "security guard",
            qualification: "Matric",
            contact: "02476543211",
            address: "malir"
        },
        {
            name: "Nadir shah",
            fatherName: "Ahad shah",
            Gender: "Male",
            maritalStatus: "unmarried",
            salary: 16000,
            age: 27,
            designation: "security guard",
            qualification: "Matric",
            contact: "03092223211",
            address: "model colony"
        },
        {
            name: "imran ali",
            fatherName: "Muhammad ibad ali",
            Gender: "Male",
            maritalStatus: "unmarried",
            designation: "security guard",
            salary: 16000,
            age: 20,
            qualification: "Matric",
            contact: "0309223456",
            address: "Clifton"
        },
        {
            name: "Noman khan",
            fatherName: "Arshad",
            Gender: "Male",
            maritalStatus: "unmarried",
            salary: 20000,
            designation: "security guard",
            age: 27,
            qualification: "Matric",
            contact: "09876543211",
            address: "Tin talwar"
        },
        {
            name: "shariq",
            fatherName: "Ali",
            Gender: "Male",
            maritalStatus: "unmarried",
            salary: 20000,
            age: 23,
            designation: "security guard",
            qualification: "Matric",
            contact: "0345698752",
            address: "Muhammad ali society"
        },
        {
            name: "Uzair khan",
            fatherName: "ahmed khan",
            Gender: "Male",
            maritalStatus: "unmarried",
            salary: 20000,
            age: 24,
            designation: "security guard",
            qualification: "No education",
            contact: "03258936211",
            address: "clifton"
        },
        {
            name: "Yasir hussain",
            fatherName: "hussain",
            Gender: "Male",
            maritalStatus: "unmarried",
            salary: 20000,
            designation: "security guard",
            age: 27,
            qualification: "Matric",
            contact: "03476543211",
            address: "defence"
        },
        {
            name: "Noman khan",
            fatherName: "Arshad",
            Gender: "Male",
            maritalStatus: "unmarried",
            salary: 40000,
            designation: "security guard",
            age: 27,
            qualification: "Matric",
            contact: "09876543211",
            address: "Tin talwar"
        },
        {
            name: "M.ali",
            fatherName: "Arshad",
            Gender: "Male",
            maritalStatus: "unmarried",
            salary: 15000,
            age: 27,
            designation: "security guard",
            qualification: "Matric",
            contact: "02476543211",
            address: "Tin talwar"
        },
        {
            name: "huzaifa khan",
            fatherName: "Arshad",
            Gender: "Male",
            maritalStatus: "unmarried",
            salary: 14500,
            age: 27,
            designation: "security guard",
            qualification: "MAtric",
            contact: "09876543211",
            address: "landhi"
        },
        {
            name: "Maqsoood Alam",
            fatherName: "Alam",
            Gender: "Male",
            maritalStatus: "unmarried",
            salary: 18000,
            age: 24,
            qualification: "Matric",
            contact: "0212345678",
            designation: "security guard",
            address: "korangi"
        },
        {
            name: "Immad ali",
            fatherName: "khursheed ali",
            Gender: "Male",
            maritalStatus: "Unmarried",
            salary: 20000,
            age: 23,
            qualification: "Matric",
            contact: "03415678904",
            address: "malir",
            designation: "security guard"
        }

    ],
    cheifs: [
        {
            name: "Noman khan",
            fatherName: "Arshad",
            Gender: "Male",
            maritalStatus: "unmarried",
            salary: 40000,
            age: 27,
            qualification: "Masters in food science",
            contact: "09876543211",
            address: "Tin talwar"
        },
        {
            name: "anabia sheikh",
            fatherName: "Asad sheikh",
            salary: 20000,
            age: 22,
            Gender: "female",
            maritalStatus: "unmarried",
            qualification: "Masters",
            contact: "09876543211",
            address: "liyari"
        },
        {
            name: "Mohsin khan",
            fatherName: "aziz khan",
            Gender: "Male",
            maritalStatus: "married",
            salary: 30000,
            age: 30,
            qualification: "Masters",
            contact: "09876543211",
            address: "Sadar"
        },
        {
            name: "sidra akram",
            fatherName: "akram",
            Gender: "female",
            maritalStatus: "married",
            salary: 30000,
            age: 28,
            qualification: "Masters",
            contact: "03976543211",
            address: "Defence"
        }
    ],

    managers: [
        {
            name: "Mohsin khan",
            fatherName: "aziz khan",
            Gender: "Male",
            maritalStatus: "married",
            salary: 50000,
            age: 30,
            designation: "event manager",
            qualification: "bachelors",
            contact: "09876543211",
            address: "Sadar"
        },
        {
            name: "Irshad khan",
            fatherName: "aziz khan",
            Gender: "Male",
            maritalStatus: "married",
            salary: 55000,
            age: 60,
            designation: "reserve manager",
            qualification: "Masters",
            contact: "09876543211",
            address: "karachi"

        },
        {
            name: "ibad ul lah",
            fatherName: "aman ullah",
            Gender: "Male",
            maritalStatus: "married",
            salary: 50000,
            age: 40,
            qualification: "Masters",
            contact: "09876543211",
            designation: "kitchen manager",
            address: "Liyari"
        },
        {

            name: "moiz khan",
            fatherName: "huzaifa khan",
            Gender: "Male",
            maritalStatus: "married",
            salary: 60000,
            age: 50,
            qualification: "bachelors",
            contact: "0390543211",
            designation: "cooking manager",
            address: "karachi"
        },
        {

            name: "faizan",
            fatherName: "imran",
            Gender: "Male",
            maritalStatus: "unmarried",
            salary: 40000,
            age: 23,
            qualification: "Masters",
            contact: "02006543211",
            designation: "area manager",
            address: "Defence"
        },
        {
            name: "aiman khan",
            fatherName: "aziz khan",
            Gender: "female",
            maritalStatus: "married",
            salary: 55000,
            age: 60,
            designation: "reserve manager",
            qualification: "Masters",
            contact: "09876543211",
            address: "karachi"
        },
        {

            name: "mahira ahmed",
            fatherName: "ahmed",
            Gender: "female",
            maritalStatus: "married",
            salary: 55000,
            age: 60,
            designation: "parties manager",
            qualification: "Masters",
            contact: "0333333211",
            address: "karachi"
        }
    ],
    events: [
        {
            name: "birthday party",
            facilities: "we give beautiful places for party and dexoration for birthday",
            cake: "we provide every type of cake"
        },
        {
            name: "conference table",
            facilities: "we give a separate place for confrerence where youi can make ur conference more effective"
        },
        {
            name: "recepitons",
            description: "klgi ayyfh9fdh heiyf gedrmk8yr dhirerbt wqewgbudisad"
        },
        {
            name: "school functions",
            description: "yyib uyr gb oaq9489b ibvw8e hu6vc"
        },
        {
            name: "office parties",
            description: "you can make your freinds happy after inviting them in beautiful place"
        }
    ],
    reservations: "you can reserve your seat before coming due to shortage of seats",
    advantages: [
        "place",
        "seaside",
        "peace",
        "family place",
        "receptions",
        "party",
        "Functions",
        "ful security",
        "play land",
        "for every type of office meeting"
    ],
    expenses: [
        "Electricity bill expenses",
        "advertismnent expense",
        "salaries expenses",
        "Decoration expenses",
        "Cooking expenses",
        "grocery expenses",
        "crocary expenses"
    ],
    menu: [
        "fish",
        "chicken",
        "beaf",
        "mutton",
        "malai boti",
        "chiken tikka",
        "chines food",
        "roll",
        "chicken roll",
        "mayo roll",
        "french fries",
        "biryani",
        "chicken biryani",
        "beaf biryani",
        "beaf botti",
        "mutton biryani",
        "mutton chops",
        "pizza",
        "pizza fries",
        "gola kabab",
        "palao",
        "yakhni",
        "soup",
        "spaghetti",
        "handi",
        "cheese naan",
        "dam qeema",
        "macroni",
        "chicken mancholian",
        "chaomin",
        "fajita",
        "tikka boti",
        "mandi",
        "chargha",
        "kaleji",
        "prawn",
        "sea foods",
        "hot dogs",
        "hot wings",
        "chicken shashlik",
        "souce and chili prawn",
        "seekh kabab",
        "beaf reshmi kabab",
        "chicken behari boti",
        "chicken beahri kabab",
        "nehari",
        "chcekn nehari",
        "lasania",
        "pasta",
        ""
    ],
    fastFood: [
        "zinger",
        "burger",
        "chicken burger",
        "beaf burger",
        "broast",
        "gol gappe",
        "chart",
        "French fries"
    ],
    deserts: [
        "coaktail",
        "kesar peda",
        "falooda",
        "rasmalai",
        "barfi",
        "chocolate cake",
        "kheer",
        "custard",
        "icecreams",
        "sweets",
        "vanila cake",
        "chocolaty desert",
        "strawberry flavour cake"
    ],
    drinks: [
        "soft drinks",
        "cold drinks",
        "lemon juice",
        "milk shakes",
        "banana shakes",
        "mango shakes",
        "strawberry shakes",
        "orange juice",
        "pista  flavour juice",
        "oreo shakes",
        "chocolate shakes",
        "tea",
        "cofee",
        "kahwa",
        "water"
    ],
    drinksPrice: [
        {
            name: "fanta",
            price: [
                {
                    regular: 50,
                    oneLtr: 80,
                    cane: 50,
                    twoLtr: 100,
                    twoAndHalfLtr: 150
                }
            ]
        },
        {
            name: "pepsi",
            price: [
                {
                    regular: 50,
                    oneLtr: 80,
                    cane: 50,
                    twoLtr: 100,
                    twoAndHalfLtr: 150
                }
            ]
        },
        {
            name: "cococola",
            price: [
                {
                    regular: 50,
                    oneLtr: 80,
                    cane: 50,
                    twoLtr: 100,
                    twoAndHalfLtr: 150
                }
            ]
        },
        {
            name: "sprit",
            price: [
                {
                    regular: 50,
                    oneLtr: 80,
                    cane: 50,
                    twoLtr: 100,
                    twoAndHalfLtr: 150
                }
            ]
        },
        {
            name: "due",
            price: [{
                regular: 50,
                oneLtr: 80,
                cane: 50,
                twoLtr: 100,
                twoAndHalfLtr: 150
            }
            ]
        },
        {
            name: "seven up",
            price: [
                {
                    regular: 50,
                    oneLtr: 80,
                    cane: 50,
                    twoLtr: 100,
                    twoAndHalfLtr: 150
                }
            ]
        }
    ],
    dishesPrice: [
        {
            name: "malai boti",
            price: 500
        },
        {
            name: "tikka",
            price: 1000
        },
        {
            name: "Fish",
            price: 800
        },
        {
            name: "Gola kabab",
            price: 500
        },
        {
            name: "chicken tandori",
            price: 1000
        },
        {
            name: "biryani",
            price: 2000
        },
        {
            name: "prawn",
            price: 700
        },
        {
            name: "malai boti",
            price: 1000
        },
        {
            name: "mayo roll",
            price: 200
        },
        {
            name: "chicken shashlik",
            price: 1500
        },
        {
            name: "cocktail",
            price: 700
        },
        {
            name: "rasmalai",
            price: 500
        },
        {
            name: "ice cream",
            price: 1000
        },
        {
            name: "sweet",
            price: 400
        },
        {
            name: "beaf kabab",
            price: 300,
        },
        {
            name: "reshmi kabab",
            price: 500
        },
        {
            name: "gola kabab",
            price: 300
        },
        {
            name: "chili prawn",
            price: 1300
        },
        {
            name: "handi",
            price: 2000

        },
        {
            name: "mandi",
            price: 1000
        },
        {
            name: "kheer",
            price: 900
        },
        {
            name: "tikka boti",
            price: 1500
        },
        {
            name: "chaomin",
            price: 500
        },
        {
            name: "chicken shuarma",
            price: 200
        },
        {
            name: "chicken mancholian",
            price: 2000
        },
        {
            name: "palao",
            price: 1700
        },
        {
            name: "cheese roll",
            price: 200
        },
        {
            name: "beahri boti",
            price: 2200
        },
        {
            name: "nihari",
            price: 500
        },
        {
            name: "chicken nihari",
            price: 700
        },
        {
            name: "chicjen balls",
            price: 300
        },
        {
            name: "paya",
            price: 2300
        },
        {
            name: "mutton",
            price: 3000
        },
        {
            name: "mutton chops",
            price: 2400
        },
        {
            name: "hot dogs",
            price: 1000
        },
        {
            name: "chicken wings",
            price: 800,
        },
        {
            name: "burger",
            price: 200
        },
        {
            name: "zinger",
            price: 500
        },
        {
            name: "beaf burger",
            price: 400
        },
        {
            name: "pizza(fajita)small",
            price: 1000
        },
        {
            name: "pizza(tikka)small",
            price: 1000
        },
        {
            name: "pizza(malai)small",
            price: 1000
        },
        {
            name: "salad",
            price: 300,
        }
    ],
    deal1: [
        {
            price: 1000,
            menu: [
                "tikka",
                "Roll",
                "coldrink",
                "fries",
            ]
        }
    ],
    deal2: [
        {
            price: 2000,
            menu: [
                "chciken shashlik",
                "chaommin",
                "coldrinks",
                "fries"
            ]
        }
    ],
    deal3: [
        {
            price: 3000,
            menu: [
                "mandi",
                "mutton chops",
                "coldrink",
                "coaktail"
            ]
        }
    ],
    deal4: [
        {
            price: 3300,
            menu: [
                "chicken mancholian",
                "desert",
                "prawn",
                "fish",
                "coldrink"
            ]
        }
    ]
    ,
    deal5: [
        {
            price: 4000,
            menu: [
                "prawn",
                "mandi",
                "chicken chaomin",
                "fries",
                "coldrink",
                "hot dogs",
                "desert"
            ]
        }
    ],
    deal6: [
        {
            price: 5000,
            menu: [
                "mandi",
                "mutton",
                'hot wings',
                "desert",
                "prawwn",
                "cheese naan",
                "coldrinks"
            ]
        }
    ],
    buffet: [
        {
            days: "only three days available for buffet",
            tuesday: [
                {
                    fourDishes: 2000,
                    fiveDishes: 3000,
                    uncountableDishes: 6000,
                }
            ],
            friday: [
                {
                    fourDishes: 1500,
                    fiveDishes: 2500,
                    uncountableDishes: 5000,
                }
            ],
            sunday: [
                {
                    fourDishes: 3000,
                    fiveDishes: 4000,
                    uncountableDishes: 7000
                }
            ]
        }
    ],
    salad: [
        {
            name: "Lasania Special Salad",
            price: 440
        },
        {
            name: "Chicken Pine Apple Salad",
            price: 410
        },
        {
            name: "Chicken Apple Salad",
            price: 410
        },
        {
            name: "Russian Salad",
            price: 340
        },
        {
            name: "Apple Cabbage salad",
            price: 250
        },
        {
            name: "Kachumar Salad",
            price: 120
        },
        {
            name: "Cole Slaw",
            price: 140
        },
        {
            name: "Fresh Green Salad",
            price: 100
        },
        {
            name: "Raita",
            price: 100
        },
        {
            name: "Plain Yogurt (Home Made)",
            price: 100
        }
    ],
    lasaniaSpecialties: [
        {
            name: "Chicken Drum Sticks  (6 Pieces)",
            price: 760
         },
        {
            name: "Chicken Wings  (10 Pieces)",
            price: 760
         },
        {
            name: "French Fries",
            price: 160
         },
        {
            name: "fish craker",
            price: 260
         },
    ],
    chineseGravys: [
        {
            name: "Lasania Special Chicken (Sizzling)",
            price: 920
         },
        {
            name: "Special Manchurian (Sizzling)",
            price: 920
         },
         {
             name: "Roast Chicken with Spicy Garlic Sauce (Sizzling)",
             price :920
         },
         {
            name: "	Chicken Green Pepper  (Sizzling)",
            price : 790
         },
         {
            name: "Chicken Manchurian (Sizzling)",
            price : 790
         },
         {
            name: "Chicken Chili Dry",
            price : 790
         },
         {
            name: "Chicken Ginger (Sizzling)",
            price : 790
         },
         {
            name: "Chicken Sweet & Sour",
            price : 770
         },
         {
            name: "Chicken Garlic Sauce  (Sizzling)",
            price : 860
         },
         {
            name: "Black Pepper Chicken (Sizzling)",
            price : 790
         }
    ],
    freeHomeDelivery: "Free Home Delivery Service is available in all over the Branches in their surrounding localities.",
    outdoorServices: "We have professional expertise in arranging outdoor parties and providing Outdoor Catering Services. We provide you from spoon to toothpick and let you fully enjoy your celeberation and give you 100% satisfaction that you did the right thing to choose kolachi to arrange your celeberation.",
    indoorServices: [
        "Birthday parties",
        "Farewell parties",
        "Meeting Area",
        "light Music",
        "reservations"
    ]
}